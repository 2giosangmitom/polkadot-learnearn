...
/**
 * Global dialog that handles 402 Payment Required responses.
 *
 * When the x402 agent detects a 402, it opens this dialog showing
 * the course name, price, and payment action. The user confirms,
 * sends PAS to the platform wallet, and the dialog calls POST /purchases
 * to verify the payment on-chain. On success, the original request
 * is automatically retried by the API client.
 */
export function X402PaymentDialog() {
  ...
  const insufficientBalance =
    balance && parseFloat(balance.formattedTransferable) < info.price;
  ...
  async function handlePurchase() {
    ...
    if (insufficientBalance) {
      toast.error(
        `Insufficient balance. You have ${balance?.formattedTransferable} PAS but need ${info.price} PAS.`,
      );
      return;
    }
    ...
    try {
      ...
      const amountInPlanck = BigInt(Math.floor(info.price * 1e10));

      toast.info("Please confirm the transaction in your wallet...");

      // Send PAS to the PLATFORM wallet (not the teacher)
      const tx = api.tx.balances.transferKeepAlive(
        info.platform_wallet_address,
        amountInPlanck,
      );
      ...
      // Verify with backend — this also triggers teacher payout
